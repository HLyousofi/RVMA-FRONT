...
import { usePostInvoice, useUpdateInvoice, useGetInvoice } from '../../services/InvoiceService';
...
const InvoiceForm = () => {
    ...
    const { data : invoice,isLoading : isLoadingWorkorder, isError : isErrorWorkorder } = useGetInvoice({id});
        ...
        if (id && invoice) {
          reset({
            customer: invoice.data.customer || '',
             vehicle: invoice.data.vehicle || '',
             expirationDate: invoice.data.expirationDate ,
            // comment: invoice.data.comment || '',
            rows: invoice.data.products
              ? invoice.data.products.map((p) => ({
    ...
    function prepareInvoiceForApi(formData) {
      
        return {
            customerId: formData.customer.id,
            vehicleId: formData.vehicle.id,
            status : invoice?.data?.status ?? 'draft',
            type : "invoice",
    ...
    const onSubmit = async (formData) => {
            // Transform the data
    const invoiceData = prepareInvoiceForApi(formData);
    if(id != null){
            try{
    ...
    if(isLoadingWorkorder){
     ...
     // Render the main content with the DataGrid
        ...
        <div className="px-6 py-6 lg:px-8">
            <h3 className="mb-4 text-xl font-medium text-gray-700 dark:text-gray-300"> Facture : { invoice?.data?.invoiceNumber }</h3>
